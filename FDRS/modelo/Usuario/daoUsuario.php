...
class DaoUsuario{
    ...
    public function iniciar($eMail,$contras){
        $sql = "SELECT * FROM usuario WHERE correo=:eMail AND contra=:contras";
        $cn = new Conexion();
        $dbh = $cn->getConexion();
        $stmt = $dbh->prepare($sql);
        ...
        if($perfil){
            ...
            echo "<h1 style='margin-left:34%;'>Correo / Contraseña incorrecto.</h1>";
            echo "</h1><br/><a style='margin-left:46%; padding:7px; background-color: #0074d9; text-decoration: none; color: white; border-radius: 10px;' href='../vista/iniciarSesion.php'>Volver a Intentar</a>";
